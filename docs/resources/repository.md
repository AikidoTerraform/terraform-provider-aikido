...
- `labels` (Attributes List) Labels managed by this resource. When set, Terraform creates/updates/deletes labels to match. Omitting labels leaves Aikido labels untouched. An empty list deletes all labels currently on the repository. (see [below for nested schema](#nestedatt--labels))
